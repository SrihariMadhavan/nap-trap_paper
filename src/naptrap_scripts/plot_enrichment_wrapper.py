...
def main():
    ...
    parser = argparse.ArgumentParser(description= "Plot the enrichment of kmer features")
    ...
    if  ('None' in sample_names) or len(sample_names) > 2:
    ...
    plot_enrichment.plot_enrichment(db=db, selector_name=selector_name,fig_save_path= fig_save_path if fig_save_path !='None' else None,sample_names=sample_names, klen = klen,fig_save_format = fig_save_format)
